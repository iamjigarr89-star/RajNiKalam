...
async function snapshot(includeDrafts=false){const sections={};for(const k of SECTIONS)sections[k]=await setting(`section:${k}`,null);const likes={};const lr=await sql`SELECT section_key,COUNT(*)::int AS count FROM section_likes GROUP BY section_key`;for(const k of SECTIONS)likes[k]=0;for(const r of lr)likes[r.section_key]=Number(r.count);return {posts:await posts(includeDrafts),socialLinks:await setting('socialLinks',{}),sections,sectionLikes:likes,photos:await setting('photos',[])}}
